...
<?php
	...
	foreach ($results->facet_counts->facet_fields->text as $facet=>$count) {
		print '<li><a onclick="waiting_on();" href="'.buildurl_addvalue($params,'text', $facet, 's',1).'">'.htmlspecialchars($facet).'</a> ('.$count.')</li>';
	}
...
<?php
	$first = true;
 	foreach ($results->facet_counts->facet_fields->text as $word=>$count) {

 		$wordlink = buildurl_addvalue($params, 'text', $word, 's', 1);
